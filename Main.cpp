...
void init()
{
	...
	endX = 550;
	endY = 550;
	end_node.position.x = endX;
	end_node.position.y = endY;
	rect(endX, endY);
	redrawSF();
	SDL_SetRenderDrawColor(_renderer, 255, 0 , 0, 255);
	srand(time(NULL));
...
void draw_path()
{
	...
	while (1)
	{
		...
		connect(up.position.x, up.position.y, down.position.x, down.position.y);
...
void rrt()
{
	...
	(rnode->position).x = rand() % _screenWidth + 1;
	(rnode->position).y = rand() % _screenHeight+ 1;
	...
	//	(nodes[index]->children).push_back(stepnode);
		...
		connect((stepnode->position).x, (stepnode->position).y, nodes[index]->position.x, nodes[index]->position.y);
		...
		//cout << to_string(node_dist(end_node.position, stepnode->position)) << endl;

		if ((notObstructed(end_node.position.x, end_node.position.y, stepnode->position.x, stepnode->position.y))&& node_dist(end_node.position, stepnode->position) < (step_size*1) )
		{
			reached = 1;
...
int main(int argc, char** argv)
{
	...
	init();
	cout << to_string(end_node.position.x) + ", " + to_string(end_node.position.y) << endl;
	while ((reached == 0))
		rrt();
	SDL_Event evnt;
	while (true) {
